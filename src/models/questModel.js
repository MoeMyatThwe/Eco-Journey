...
module.exports.createQuest = (data, callback) => {
  const SQLSTATEMENT = `
        INSERT INTO Quest(title,description,difficulty_level,reward_experience_points,reward_gold,reward_badge_id)
        VALUES (?,?,?,?,?,?);
        `;
  const VALUES = [
    data.title,
...
module.exports.readQuestById = (data, callback) => {
  const SQLSTATEMENT = `SELECT q.*, b.*
FROM quest q
JOIN badge b ON q.reward_badge_id = b.badge_id WHERE quest_id = ?;`;
...
module.exports.updateQuestById = (data, callback) => {
  const SQLSTATEMENT = `UPDATE Quest SET title = ?, description = ?, difficulty_level = ?, reward_experience_points = ?, reward_gold = ?, reward_badge_id = ? WHERE quest_id = ?;`;
  const VALUES = [
    data.title,
    data.description,
    data.difficulty_level,
